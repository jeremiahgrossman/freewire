...
- The tunnel is established before the captive portal is satisfied — Freewire does not require the user to authenticate with the captive portal first. **Status (2026-08-22): the machinery is built and each piece is proven; end-to-end on a real portal is not yet confirmed.** What works: the DNS tunnel zone is delegated (`t.pinghop.net`) and the carrier sustains ~400–500 Kbps at 0% loss through both a public resolver and Starbucks' own resolver (measured); the client now falls back to a cached registration when a portal blocks the control-plane API, so it reaches the DNS tunnel instead of surrendering to a login screen (proven in the config7 locked-portal simulation). What is not confirmed: a real pre-login café run reached the DNS tunnel but carried no traffic — traced to a false-Protected bug (now fixed: sustained-egress verification) compounded by accumulated local routing cruft, not to the DNS tunnel being incapable (the carrier sustains fine). A clean-slate real-portal retest is the remaining step to call this requirement met. Until then, the reliable path on a portal is: detect, open the login page, connect afterwards.
...
- The active tunnel path (DNS, HTTP CONNECT, or TLS/443) is displayed in the client for transparency. **Superseded:** the shipped client never surfaces which carrier is active anywhere in the UI. `PanelView` shows only connection state (Protected / Limited connectivity / error), never a carrier name. The only place a carrier name is visible at all is a diagnostic file (`/var/run/freewire-tunnel.status`, added 2026-08-29 for field testing) and `testing/cafe-measure.sh`'s output — not user-facing.

**Protocol Fallback Chain:**

> **Superseded (2026-08-30).** This section describes the original 4-path design. The shipped chain has grown to **nine carriers**, attempted in this order: `wireguard` → `udp443` → `http_connect` → `tls443` → `wss443` → `cdn_wss` → `dns_tcp` → `dns` → `icmp_udp`. The four below still exist under different names (`http_connect`, `tls443`, `dns`→now the *fallback within* the DNS family, `icmp_udp`), but five carriers were added after field testing found real captive portals gate by *destination*, not just by port: `wireguard`/`udp443` (near line-rate UDP, tried first on networks that don't block it at all), `wss443` (WireGuard inside WebSocket framing — passes portals that intercept raw TLS/443 but allow a real HTTP upgrade), `cdn_wss` (the same, fronted through a CloudFront edge, for portals that allow-list specific third-party destinations but block the server's own IP), and `dns_tcp` (WireGuard over a TCP connection to port 53 — added 2026-08-28, field-validated 2026-08-30 at a real destination-gated café: it gets real TCP backpressure instead of the UDP DNS carrier's tail-drop-and-collapse failure mode, and measured 3.6–7.6 Mbps in the field vs. the UDP DNS carrier's ~72 Kbps). See `CLAUDE.md`'s Current State for the full, continuously updated history, and `technical-architecture.md` (also stale — see its own correction note).
...
Once any path establishes a tunnel, the client renegotiates: if a faster path is now reachable through the tunnel, it upgrades transparently (in the shipped client, `PathUpgradeManager` only probes `.dns` from ICMP and `.udp443` from any slower carrier — it does not treat every carrier as an upgrade candidate; see the correction in `path-upgrade-manager-spec.md`). The user is never asked to choose a path.
