...
**Revised 2026-08-30** to match what actually happened, not the original plan.
Six phases now, not four — two phases the original plan didn't anticipate
(field hardening, and the deferral of self-hosted distribution) are folded in
directly rather than left as a footnote above a stale table.

| Phase | What | Status | Specs to read |
|---|---|---|---|
| **1 — Foundation** | Device key lifecycle, WireGuard on open network, TLS/443 managed connection, basic macOS UX (menu bar app) | **Done.** | `engineering-handoff.md`, `ux-workflows.md` §3, `client-server-api-spec.md`, `data-model.md`, `error-states-spec.md` |
| **2 — Captive portal bypass** | 9-carrier fallback chain (`wireguard`, `udp443`, `http_connect`, `tls443`, `wss443`, `cdn_wss`, `dns_tcp`, `dns`, `icmp_udp`), path upgrade manager, Essentials Mode, peer-table persistence | **Done — exceeded the original scope.** Planned as 4 paths; shipped 9 carriers plus two whole features (Essentials Mode, peer persistence) the original plan never anticipated. Field-validated at multiple real cafés, not simulated configs. | `technical-architecture.md`, `dns-tunnel-protocol-spec.md`, `icmp-tunnel-protocol-spec.md`, `path-upgrade-manager-spec.md`, `ESSENTIALS-MODE-SPEC.md` — all carry inline 2026-08-30 correction notes; read those, not the original body text |
...
| **6 — Queued / remaining work** | Kill switch (blocked on Developer ID); EBS snapshot policy (IAM+DLM prepped 2026-08-30, blocked on IAM-admin access to apply — see `deploy/dlm-snapshot-setup.md`) + CloudWatch alarm + SSH-ingress-refresh fix (still not started); Essentials Mode field validation (desk pre-flight re-confirmed current); `dns_tcp` as a `PathUpgradeManager` candidate (**done 2026-08-30** — also fixed a real bug found while scoping it: the Swift app had no `dns_tcp` representation at all and silently mislabeled it as `wireguard`); IPv6 carrier (**carrier half staged 2026-08-30**, not active — the leak-sensitive routing half is deliberately deferred to a real v6 network, confirmed with the user) | **Mixed** — see each item; no longer uniformly "not started." | — |
